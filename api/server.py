...
# Manual player upload endpoint
@api_router.post("/admin/upload-manual-players/{slot}")
async def admin_upload_manual_players(slot: int, user_id: str = Query(...), players_text: str = Body(...)):
    """Upload manual player list before ESPN data is available.
    Format: Name,Price (one per line) or Name (prices can be set later)
    """
    await check_admin(user_id)
    t = await db.tournaments.find_one({"slot": slot}, {"_id": 0})
    if not t:
    ...
    # Parse the input
    lines = [line.strip() for line in players_text.strip().split('\n') if line.strip()]
    manual_golfers = []
    ...
    # Merge with existing golfers (keep any ESPN ones, replace manual ones)
    existing = t.get("golfers", [])
    espn_golfers = [g for g in existing if g.get("source") == "espn"]
    all_golfers = espn_golfers + manual_golfers
    
    await db.tournaments.update_one(
        {"slot": slot},
        {"$set": {"golfers": all_golfers, "status": "golfers_loaded"}}
    )
    
    return {"message": f"Uploaded {len(manual_golfers)} manual players", "count": len(manual_golfers)}


# Get mapping suggestions when ESPN data loads
@api_router.get("/admin/mapping-suggestions/{slot}")
async def admin_get_mapping_suggestions(slot: int, user_id: str = Query(...)):
    """Get suggested mappings between manual and ESPN players using fuzzy matching."""
    await check_admin(user_id)
    from difflib import SequenceMatcher
    
    t = await db.tournaments.find_one({"slot": slot}, {"_id": 0})
    if not t or not t.get("golfers"):
        raise HTTPException(status_code=404, detail="Tournament or golfers not found")
    
    golfers = t["golfers"]
    manual_players = [g for g in golfers if g.get("source") == "manual"]
    espn_players = [g for g in golfers if g.get("source") == "espn"]
    
    if not manual_players:
        return {"mappings": [], "message": "No manual players to map"}
    
    if not espn_players:
        return {"mappings": [], "message": "No ESPN data loaded yet", "manual_count": len(manual_players)}
    
    # Calculate similarity for each manual player
    suggestions = []
    for manual in manual_players:
        manual_name = manual["name"].lower()
        best_match = None
        best_score = 0
        
        for espn in espn_players:
            espn_name = espn["name"].lower()
            # Use SequenceMatcher for fuzzy matching
            score = SequenceMatcher(None, manual_name, espn_name).ratio()
            if score > best_score:
                best_score = score
                best_match = espn
        
        status = "auto" if best_score >= 0.9 else "suggested" if best_score >= 0.7 else "unmapped"
        
        suggestions.append({
            "manual_name": manual["name"],
            "espn_id": best_match["espn_id"] if best_match else None,
            "espn_name": best_match["name"] if best_match else None,
            "confidence": round(best_score, 2),
            "status": status,
            "manual_price": manual.get("price")
        })
    
    return {
        "mappings": suggestions,
        "manual_count": len(manual_players),
        "espn_count": len(espn_players),
        "all_espn_players": [{"espn_id": e["espn_id"], "name": e["name"]} for e in espn_players]
    }


# Apply mappings and update golfers
@api_router.post("/admin/apply-mappings/{slot}")
async def admin_apply_mappings(slot: int, user_id: str = Query(...), mappings: List[Dict[str, Any]] = Body(...)):
    """Apply manual-to-ESPN player mappings. 
    Mappings format: [{"manual_name": "...", "espn_id": "...", "espn_name": "..."}]
    """
    await check_admin(user_id)
    ...
    mapping_dict = {m["manual_name"]: m for m in mappings}
    
    # Update golfers: replace manual players with their ESPN equivalents
    updated_golfers = []
    mapped_count = 0
    
    for g in golfers:
        if g.get("source") == "manual" and g["name"] in mapping_dict:
            mapping = mapping_dict[g["name"]]
            # Find the ESPN player
            espn_player = next((e for e in golfers if e.get("espn_id") == mapping["espn_id"] and e.get("source") == "espn"), None)
            if espn_player:
                # Keep manual price if it was set
                if g.get("price"):
                    espn_player["price"] = g["price"]
                espn_player["mapping_status"] = "mapped"
                updated_golfers.append(espn_player)
                mapped_count += 1
                
                # Save mapping to audit table
                await db.player_mappings.insert_one({
                    "tournament_id": t["id"],
                    "manual_name": g["name"],
                    "espn_id": mapping["espn_id"],
                    "espn_name": mapping["espn_name"],
                    "mapped_by": user_id,
                    "mapped_at": datetime.now(timezone.utc).isoformat(),
                    "confidence_score": mapping.get("confidence", 0)
                })
        elif g.get("source") == "espn":
            # Skip if this ESPN player was already added as a mapped manual player
            if not any(m["espn_id"] == g.get("espn_id") for m in mappings):
                updated_golfers.append(g)
        elif g.get("source") == "manual":
            # Unmapped manual player - keep for now (admin might want to delete later)
            g["mapping_status"] = "unmapped"
            updated_golfers.append(g)
    
    # Remove duplicates by espn_id (keep first occurrence)
    seen = set()
    final_golfers = []
    for g in updated_golfers:
        key = g.get("espn_id") or g.get("name")
        if key not in seen:
            seen.add(key)
            final_golfers.append(g)
    
    await db.tournaments.update_one(
        {"slot": slot},
        {"$set": {"golfers": final_golfers}}
    )
    
    return {"message": f"Mapped {mapped_count} players", "total_golfers": len(final_golfers)}


# Delete unmapped manual players
@api_router.delete("/admin/delete-unmapped/{slot}")
async def admin_delete_unmapped_manual(slot: int, user_id: str = Query(...)):
    """Delete all unmapped manual players (cleanup after mapping)."""
    await check_admin(user_id)
    ...
    golfers = t.get("golfers", [])
    cleaned = [g for g in golfers if not (g.get("source") == "manual" and g.get("mapping_status") == "unmapped")]
    deleted = len(golfers) - len(cleaned)
    ...
    return {"message": f"Deleted {deleted} unmapped manual players"}
